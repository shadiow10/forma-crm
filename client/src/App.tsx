...
const registrations: Record<number, Registration[]> = {
  1: [{ id: 101, formation: "Design graphique", groupe: "Design graphique — Après-midi", date: "10/03/2026", status: "En cours", total: 85000, paid: 85000, notes: "" }],
  2: [{ id: 102, formation: "Développement web", groupe: "Web Frontend — Soir", date: "02/02/2026", status: "En cours", total: 95000, paid: 70000, notes: "Paiement en deux fois" }],
  3: [{ id: 103, formation: "Comptabilité pratique", groupe: "Comptabilité pratique — Matin", date: "01/09/2026", status: "Inscrit", total: 62000, paid: 31000, notes: "" }, { id: 104, formation: "Bureautique & Excel", groupe: "Excel avancé — Matin", date: "05/01/2026", status: "Terminé", total: 45000, paid: 45000, notes: "" }],
  5: [{ id: 105, formation: "Marketing digital", groupe: "Marketing digital — Après-midi", date: "11/02/2026", status: "Terminé", total: 70000, paid: 70000, notes: "" }],
};
const documents: Record<number, StudentDocument[]> = {
  1: [{ id: 1, type: "Carte d'identité", file: "CNI_Lina_Haddad.pdf", status: "Validé", date: "10 mars 2026" }, { id: 2, type: "Photo d'identité", file: "Photo_Lina_Haddad.jpg", status: "Validé", date: "10 mars 2026" }],
  2: [{ id: 3, type: "Carte d'identité", file: "CNI_Yacine_Merabet.pdf", status: "Validé", date: "02 févr. 2026" }, { id: 4, type: "Diplôme / justificatif", file: "Diplome_Yacine.pdf", status: "À vérifier", date: "02 févr. 2026" }],
  3: [{ id: 5, type: "Carte d'identité", file: "CNI_Sara_Belkacem.pdf", status: "Validé", date: "01 sept. 2026" }],
};

const teachersSeed: Teacher[] = [
  { id: 1, name: "Nadia Benali", initials: "NB", subject: "Bureautique & Excel", phone: "0555 44 10 21", email: "nadia.benali@formaplus.dz", classes: ["Excel avancé — Matin", "Bureautique — Soir"], rate: 1800, contract: "Temps plein", color: "#5B8DEF" },
  { id: 2, name: "Omar Cherif", initials: "OC", subject: "Développement web", phone: "0661 20 38 11", email: "omar.cherif@formaplus.dz", classes: ["Web Frontend — Soir"], rate: 2400, contract: "Temps partiel", color: "#E57865" },
  { id: 3, name: "Samira Ait Ali", initials: "SA", subject: "Comptabilité", phone: "0770 14 62 20", email: "samira.aitali@formaplus.dz", classes: ["Comptabilité pratique — Matin", "Comptabilité — Week-end"], rate: 2100, contract: "Temps plein", color: "#E4A853" },
  { id: 4, name: "Yanis Boudiaf", initials: "YB", subject: "Design graphique", phone: "0550 17 83 04", email: "yanis.boudiaf@formaplus.dz", classes: ["Design graphique — Après-midi"], rate: 2300, contract: "Temps partiel", color: "#B56FCE" },
  { id: 5, name: "Riad Mansouri", initials: "RM", subject: "Marketing digital", phone: "0698 04 72 10", email: "riad.mansouri@formaplus.dz", classes: ["Marketing digital — Après-midi"], rate: 2200, contract: "Temps partiel", color: "#2A9D8F" },
];

const classesSeed: SchoolClass[] = [
...
type DataStore = {
  courses: Course[]; setCourses: (next: Course[]) => void;
  classes: SchoolClass[]; setClasses: (next: SchoolClass[]) => void;
  teachers: Teacher[]; setTeachers: (next: Teacher[]) => void;
};
const DataContext = createContext<DataStore>({ courses: coursesSeed, setCourses: () => {}, classes: classesSeed, setClasses: () => {}, teachers: teachersSeed, setTeachers: () => {} });
...
type FieldDef = { key: string; label: string; type?: "text" | "number" | "select" | "textarea"; options?: string[]; placeholder?: string; required?: boolean; full?: boolean };

function FormModal({ title, subtitle, fields, initial, submitLabel, onClose, onSubmit }: { title: string; subtitle?: string; fields: FieldDef[]; initial: Record<string, string>; submitLabel: string; onClose: () => void; onSubmit: (values: Record<string, string>) => void }) {
  const [values, setValues] = useState<Record<string, string>>(initial);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const set = (key: string, value: string) => { setValues((prev) => ({ ...prev, [key]: value })); setErrors((prev) => ({ ...prev, [key]: "" })); };
  const submit = () => {
    const next: Record<string, string> = {};
    fields.forEach((field) => { if (field.required && !String(values[field.key] || "").trim()) next[field.key] = "Requis"; });
    if (Object.keys(next).length) { setErrors(next); return; }
    onSubmit(values);
  };
  return <div className="drawer-layer" onClick={onClose}><div className="form-modal" onClick={(event) => event.stopPropagation()}>
    <div className="form-modal-head"><div><span className="eyebrow">{subtitle || "Formulaire"}</span><h2>{title}</h2></div><button className="close-button" onClick={onClose}><Icon name="close" size={20} /></button></div>
    <div className="form-modal-body"><div className="form-modal-grid">{fields.map((field) => <div className={`form-field ${field.full ? "full" : ""}`} key={field.key}>
      <label>{field.label}{field.required && " *"}</label>
      {field.type === "select" ? <select className="field" value={values[field.key] || ""} onChange={(event) => set(field.key, event.target.value)}>{(field.options || []).map((option) => <option key={option}>{option}</option>)}</select>
        : field.type === "textarea" ? <textarea className="field textarea" value={values[field.key] || ""} onChange={(event) => set(field.key, event.target.value)} placeholder={field.placeholder} />
        : <input className="field" type={field.type === "number" ? "number" : "text"} value={values[field.key] || ""} onChange={(event) => set(field.key, event.target.value)} placeholder={field.placeholder} />}
      {errors[field.key] && <span className="field-error">{errors[field.key]}</span>}
    </div>)}</div></div>
    <div className="form-modal-foot"><Button variant="outline" onClick={onClose}>Annuler</Button><Button icon="check" onClick={submit}>{submitLabel}</Button></div>
  </div></div>;
}
...
function Inscriptions({ contacts, setContacts, setSelected, setNotice }: { contacts: Person[]; setContacts: (next: Person[]) => void; setSelected: (person: Person) => void; setNotice: (message: string) => void }) {
...
function Students({ contacts, setContacts, setSelected, setNotice, goTo, canOpen }: { contacts: Person[]; setContacts: (next: Person[]) => void; setSelected: (person: Person) => void; setNotice: (message: string) => void; goTo: (key: PageKey) => void; canOpen: (key: PageKey) => boolean }) {
  ...
  const [showNew, setShowNew] = useState(false);
  const statusFor = (person: Person) => stageLabels[person.stage];
  const filtered = contacts.filter((person) => `${person.name} ${person.email} ${person.phone} ${person.course}`.toLowerCase().includes(query.toLowerCase()) && (statusFilter === "Tous" || statusFor(person) === statusFilter) && (courseFilter === "Toutes" || person.course === courseFilter));
  return <>
...
function NewStudentDrawer({ onClose, onCreated }: { onClose: () => void; onCreated: (name: string, phone: string, email: string) => void }) {
  const [name, setName] = useState("");
  const [phone, setPhone] = useState("");
  const [email, setEmail] = useState("");
  return <Drawer title="Nouvel étudiant" onClose={onClose}><div className="drawer-intro"><span className="activity-icon" style={{ background: COLORS.tealLight, color: COLORS.teal }}><Icon name="graduation" size={16} /></span><div><strong>Créer une fiche étudiant</strong><span>Les informations essentielles suffisent pour commencer.</span></div></div><div className="drawer-section"><h4>Informations essentielles</h4><div className="settings-form"><label>Nom complet *<input className="field" autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="Ex. Amine Khelifi" /></label><label>Téléphone *<input className="field" value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="0555 00 00 00" /></label><label>Email<input className="field" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="email@exemple.com" /></label></div></div><div className="drawer-section"><h4>Informations complémentaires <span className="muted-label">Optionnel</span></h4><div className="settings-form"><label>Date de naissance<input className="field" placeholder="JJ/MM/AAAA" /></label><label>Adresse<input className="field" placeholder="Adresse de résidence" /></label></div></div><div className="drawer-footer-actions"><Button variant="outline" onClick={onClose}>Annuler</Button><Button disabled={!name.trim() || !phone.trim()} onClick={() => onCreated(name.trim(), phone.trim(), email.trim())}>Créer l'étudiant</Button></div></Drawer>;
}

function Classes({ setSelectedClass, setNotice }: { setSelectedClass: (item: SchoolClass) => void; setNotice: (message: string) => void }) {
  const { classes, setClasses, teachers, courses } = useData();
  const [showNew, setShowNew] = useState(false);
  const days = ["Dim", "Lun", "Mar", "Mer", "Jeu", "Ven", "Sam"];
  const palette = ["#5B8DEF", "#E4A853", "#B56FCE", "#2A9D8F", "#E57865", "#6A63B8"];
  const createGroup = (values: Record<string, string>) => {
    const cap = parseInt(values.capacity || "15") || 15;
    setClasses([...classes, { id: Date.now(), name: values.name, teacher: values.teacher, room: values.room || "À définir", schedule: values.schedule || "À planifier", days: (values.day ? [values.day] : ["Lun"]), time: values.time || "09:00", enrolled: 0, capacity: cap, status: "Ouvert", subject: values.subject || (courses[0]?.short || "Général"), color: palette[classes.length % palette.length], roster: [] }]);
    setShowNew(false);
    setNotice(`Groupe « ${values.name} » créé.`);
  };
  return <>
    {showNew && <FormModal title="Créer un groupe" subtitle="Nouveau groupe" submitLabel="Créer le groupe" onClose={() => setShowNew(false)} onSubmit={createGroup}
      initial={{ name: "", teacher: teachers[0]?.name || "", room: "", day: "Lun", time: "09:00", schedule: "", capacity: "15", subject: courses[0]?.short || "" }}
      fields={[
        { key: "name", label: "Nom du groupe", required: true, placeholder: "Ex. Excel avancé — Matin", full: true },
        { key: "teacher", label: "Formateur", type: "select", options: teachers.map((t) => t.name) },
        { key: "subject", label: "Matière", type: "select", options: courses.map((c) => c.short) },
        { key: "room", label: "Salle", placeholder: "Salle A1" },
        { key: "capacity", label: "Capacité", type: "number", placeholder: "15" },
        { key: "day", label: "Jour", type: "select", options: ["Dim", "Lun", "Mar", "Mer", "Jeu", "Ven", "Sam"] },
        { key: "time", label: "Heure", type: "select", options: ["09:00", "14:00", "18:00"] },
        { key: "schedule", label: "Horaire (texte)", placeholder: "Dim · Mar · Jeu, 09:00–11:00", full: true },
      ]} />}
    <PageHeader eyebrow="Organisation pédagogique" title="Groupes" description="Une vue claire des groupes, des salles, des formateurs et des capacités." action actionLabel="Créer un groupe" onAction={() => setShowNew(true)} />
...
function Formations({ setNotice }: { setNotice: (message: string) => void }) {
  ...
  const [showNew, setShowNew] = useState(false);
  const palette = ["#5B8DEF", "#E4A853", "#B56FCE", "#2A9D8F", "#E57865", "#6A63B8"];
  const createCourse = (values: Record<string, string>) => {
    const price = parseInt(values.price || "0") || 0;
    setCourses([...courses, { id: Date.now(), name: values.name, short: values.short || values.name.split(" ")[0], duration: values.duration || "3 mois", price, students: 0, color: palette[courses.length % palette.length] }]);
    setShowNew(false);
    setNotice(`Formation « ${values.name} » ajoutée au catalogue.`);
  };
  const exportCatalogue = () => downloadCSV("catalogue-formations.csv", [["Formation", "Durée", "Prix (DA)", "Étudiants"], ...courses.map((c) => [c.name, c.duration, c.price, c.students])]);
  return <>
    {showNew && <FormModal title="Nouvelle formation" subtitle="Catalogue" submitLabel="Ajouter la formation" onClose={() => setShowNew(false)} onSubmit={createCourse}
      initial={{ name: "", short: "", duration: "3 mois", price: "" }}
      fields={[
        { key: "name", label: "Nom de la formation", required: true, placeholder: "Ex. Développement mobile", full: true },
        { key: "short", label: "Nom court", placeholder: "Mobile" },
        { key: "duration", label: "Durée", type: "select", options: ["1 mois", "2 mois", "3 mois", "4 mois", "6 mois", "9 mois", "12 mois"] },
        { key: "price", label: "Prix mensuel (DA)", type: "number", required: true, placeholder: "45000", full: true },
      ]} />}
    <PageHeader eyebrow="Catalogue pédagogique" title="Formations" description="Pilotez votre catalogue, les tarifs et les places disponibles par parcours." action actionLabel="Nouvelle formation" onAction={() => setShowNew(true)} />
...
function Planning({ setSelectedClass, setNotice }: { setSelectedClass: (item: SchoolClass) => void; setNotice: (message: string) => void }) {
  ...
  const days = ["Dim", "Lun", "Mar", "Mer", "Jeu", "Ven", "Sam"];
  const palette = ["#5B8DEF", "#E4A853", "#B56FCE", "#2A9D8F", "#E57865", "#6A63B8"];
  const createSession = (values: Record<string, string>) => {
    setClasses([...classes, { id: Date.now(), name: values.name, teacher: values.teacher, room: values.room || "À définir", schedule: `${values.day}, ${values.time}`, days: [values.day], time: values.time, enrolled: 0, capacity: parseInt(values.capacity || "15") || 15, status: "Ouvert", subject: courses[0]?.short || "Général", color: palette[classes.length % palette.length], roster: [] }]);
    setShowNew(false);
    setNotice(`Séance « ${values.name} » ajoutée au planning.`);
  };
  return <>
    {showNew && <FormModal title="Ajouter une séance" subtitle="Planning" submitLabel="Ajouter la séance" onClose={() => setShowNew(false)} onSubmit={createSession}
      initial={{ name: "", teacher: teachers[0]?.name || "", room: "", day: "Lun", time: "09:00", capacity: "15" }}
      fields={[
        { key: "name", label: "Nom de la séance", required: true, placeholder: "Ex. Excel avancé — Matin", full: true },
        { key: "teacher", label: "Formateur", type: "select", options: teachers.map((t) => t.name) },
        { key: "room", label: "Salle", placeholder: "Salle A1" },
        { key: "day", label: "Jour", type: "select", options: ["Dim", "Lun", "Mar", "Mer", "Jeu", "Ven", "Sam"] },
        { key: "time", label: "Heure", type: "select", options: ["09:00", "14:00", "18:00"] },
    ...
    <PageHeader eyebrow="Organisation pédagogique" title="Planning" description="Visualisez les séances de la semaine et repérez les créneaux encore disponibles." action actionLabel="Ajouter une séance" onAction={() => setShowNew(true)} />
...
function Attendance({ attendance, setAttendance, setNotice }: { attendance: Record<number, string[]>; setAttendance: (next: Record<number, string[]>) => void; setNotice: (message: string) => void }) {
  ...
  return <>
    <PageHeader eyebrow="Suivi des séances" title="Présences" description="Enregistrez la présence en quelques secondes et détectez les risques tôt." action actionLabel="Exporter la feuille" actionIcon="download" onAction={() => { downloadCSV(`presences-${selected.name.replace(/[^a-z0-9]/gi, "-").toLowerCase()}.csv`, [["Étudiant", ...attendanceDates, "Taux"], ...roster.map((person) => { const values = attendance[person.id] || attendanceSeed[person.id] || attendanceDates.map(() => "P"); return [person.name, ...values.map((v) => v === "P" ? "Présent" : v === "L" ? "Retard" : "Absent"), `${attendanceRate(values)}%`]; })]); setNotice("Feuille de présence exportée en CSV."); }} />
...
function Payments({ setNotice }: { setNotice: (message: string) => void }) {
  ...
  return <>
    <PageHeader eyebrow="Suivi financier" title="Paiements & finances" description="Gardez une vision précise des encaissements et des soldes à recouvrer." action actionLabel="Télécharger le rapport" actionIcon="download" onAction={() => { downloadCSV("rapport-paiements.csv", [["Étudiant", "Formation", "Total", "Payé", "Solde", "Date", "Méthode", "Statut"], ...rows.map((r) => [r.student, r.course, r.total, r.paid, r.balance, r.date, r.method, r.status])]); setNotice("Rapport financier téléchargé en CSV."); }} />
...
function Teachers({ setSelectedTeacher, setNotice }: { setSelectedTeacher: (teacher: Teacher) => void; setNotice: (message: string) => void }) {
  const { classes, teachers, setTeachers, courses } = useData();
  const [showNew, setShowNew] = useState(false);
  const palette = ["#5B8DEF", "#E4A853", "#B56FCE", "#2A9D8F", "#E57865", "#6A63B8"];
  const createTeacher = (values: Record<string, string>) => {
    setTeachers([...teachers, { id: Date.now(), name: values.name, initials: values.name.split(" ").map((w) => w[0]).slice(0, 2).join("").toUpperCase(), subject: values.subject, phone: values.phone || "—", email: values.email || "—", classes: [], rate: parseInt(values.rate || "2000") || 2000, contract: values.contract || "Temps partiel", color: palette[teachers.length % palette.length] }]);
    setShowNew(false);
    setNotice(`Formateur ${values.name} ajouté à l'équipe.`);
  };
  return <>
    {showNew && <FormModal title="Ajouter un formateur" subtitle="Équipe pédagogique" submitLabel="Ajouter le formateur" onClose={() => setShowNew(false)} onSubmit={createTeacher}
      initial={{ name: "", subject: courses[0]?.name || "", phone: "", email: "", rate: "2000", contract: "Temps partiel" }}
      fields={[
        { key: "name", label: "Nom complet", required: true, placeholder: "Ex. Karim Belaid", full: true },
        { key: "subject", label: "Matière", type: "select", options: courses.map((c) => c.name) },
        { key: "contract", label: "Contrat", type: "select", options: ["Temps plein", "Temps partiel", "Vacataire"] },
        { key: "phone", label: "Téléphone", placeholder: "0555 00 00 00" },
        { key: "email", label: "Email", placeholder: "prenom.nom@formaplus.dz" },
        { key: "rate", label: "Tarif horaire (DA)", type: "number", placeholder: "2000", full: true },
      ]} />}
    <PageHeader eyebrow="Équipe pédagogique" title="Formateurs" description="Un annuaire clair pour piloter l'équipe et les heures enseignées." action actionLabel="Ajouter un formateur" onAction={() => setShowNew(true)} />
...
function PersonDrawer({ person, onClose, setNotice, onAddNote }: { person: Person; onClose: () => void; setNotice: (message: string) => void; onAddNote: (note: string) => void }) {
  ...
  const status = stageLabels[person.stage];
  const studentRegistrations = registrations[person.id] || (person.enrolled ? [{ id: 900 + person.id, formation: person.course, groupe: person.teacher ? `${person.course} — Groupe principal` : "Groupe à affecter", date: person.enrolled, status: status as Registration["status"], total: person.balance + (person.payment === "Payé" ? 0 : person.balance), paid: Math.max(0, (person.balance ? 0 : 1)), notes: "" }] : []);
  const studentDocuments = documents[person.id] || [];
  return <Drawer title={person.name} onClose={onClose}><div className="profile-hero"><span className="profile-avatar large">{initials(person.name)}</span><div><div className="eyebrow">FICHE ÉTUDIANT</div><h2>{person.name}</h2><div className="profile-subline"><span>Étudiant</span><Badge tone={statusTone(status)} dot>{status}</Badge></div></div><Button variant="outline" icon="calendar" onClick={() => setNotice("Planning de l'étudiant ouvert.")}>Planning</Button></div><div className="profile-summary-grid"><div><span>Présence</span><strong>{person.attendance || 0}%</strong><small>sur les séances suivies</small></div><div><span>Paiement</span><strong>{formatMoney(Math.max(0, studentRegistrations.reduce((sum, item) => sum + item.paid, 0)))}</strong><small>{formatMoney(studentRegistrations.reduce((sum, item) => sum + item.total - item.paid, 0))} restant</small></div><div><span>Formation</span><strong>{studentRegistrations.length}</strong><small>inscription{studentRegistrations.length > 1 ? "s" : ""}</small></div><div><span>Certificat</span><strong>{person.stage === "diplome" ? "Disponible" : "À venir"}</strong><small>{person.stage === "diplome" ? "à télécharger" : "non encore délivré"}</small></div></div><div className="drawer-section profile-section"><div className="drawer-section-head"><h4>Informations personnelles</h4><Button variant="ghost" onClick={() => setNotice("Modification de la fiche étudiant simulée.")}>Modifier</Button></div><div className="info-grid"><div><span>Nom complet</span><strong>{person.name}</strong></div><div><span>Téléphone</span><strong>{person.phone}</strong></div><div><span>Email</span><strong>{person.email}</strong></div><div><span>Date de naissance</span><strong>{person.dob || "Non renseignée"}</strong></div><div><span>Adresse</span><strong>{person.address || "Non renseignée"}</strong></div><div><span>Date d'inscription</span><strong>{person.enrolled || "À compléter"}</strong></div></div></div><div className="drawer-section profile-section"><div className="drawer-section-head"><div><h4>Documents</h4><span className="section-caption">Documents de {person.name}</span></div><Button variant="soft" icon="plus" onClick={() => setNotice("Ajout de document simulé — le stockage sera connecté ultérieurement.")}>Ajouter un document</Button></div>{studentDocuments.length ? <div className="document-list">{studentDocuments.map((doc) => <div className="document-row" key={doc.id}><span className="document-icon"><Icon name="file" size={16} /></span><div><strong>{doc.type}</strong><span>{doc.file} · ajouté le {doc.date}</span></div><Badge tone={doc.status === "Validé" ? "success" : "warning"}>{doc.status}</Badge><button className="text-button" onClick={() => setNotice(`${doc.file} ouvert en aperçu.`)}>Voir</button><button className="row-menu" onClick={() => setNotice(`${doc.file} prêt à être téléchargé.`)}><Icon name="download" size={15} /></button></div>)}</div> : <div className="empty-state"><span className="activity-icon"><Icon name="file" size={16} /></span><div><strong>Aucun document ajouté</strong><span>Ajoutez la carte d'identité ou un justificatif.</span></div></div>}</div><div className="drawer-section profile-section"><div className="drawer-section-head"><div><h4>Inscriptions</h4><span className="section-caption">Une fiche peut contenir plusieurs inscriptions.</span></div><Button icon="plus" onClick={() => setShowRegistration(true)}>Créer une inscription</Button></div>{studentRegistrations.length ? <div className="registration-list">{studentRegistrations.map((registration) => <div className="registration-card" key={registration.id}><div className="registration-card-head"><div><strong>{registration.formation}</strong><span>{registration.groupe}</span></div><Badge tone={statusTone(registration.status)} dot>{registration.status}</Badge></div><div className="registration-meta"><span>Inscrit le <strong>{registration.date}</strong></span><span>Total <strong>{formatMoney(registration.total)}</strong></span><span>Payé <strong>{formatMoney(registration.paid)}</strong></span><span>Restant <strong className={registration.total - registration.paid > 0 ? "money-danger" : "money-ok"}>{formatMoney(Math.max(0, registration.total - registration.paid))}</strong></span></div></div>)}</div> : <div className="empty-state"><span className="activity-icon"><Icon name="file" size={16} /></span><div><strong>Aucune inscription</strong><span>Créez une inscription pour associer une formation et un groupe.</span></div><Button icon="plus" onClick={() => setShowRegistration(true)}>Créer</Button></div>}</div><div className="profile-preview-grid"><div className="drawer-section profile-section"><h4>Présences</h4><div className="preview-stat"><strong>{person.attendance || 0}%</strong><span>{person.attendance < 75 && person.attendance > 0 ? "À surveiller" : "Assiduité régulière"}</span></div><div className="mini-progress"><i style={{ width: `${person.attendance}%` }} /></div></div><div className="drawer-section profile-section"><h4>Paiements</h4><div className="preview-stat"><strong>{person.payment}</strong><span>{formatMoney(person.balance)} restant</span></div></div><div className="drawer-section profile-section"><h4>Certificats</h4><div className="preview-stat"><strong>{person.stage === "diplome" ? "Disponible" : "Non délivré"}</strong><span>{person.stage === "diplome" ? "Ouvrir le certificat" : "Après formation terminée"}</span></div></div></div><div className="drawer-section"><h4>Ajouter une note</h4><textarea className="field textarea" value={note} onChange={(event) => setNote(event.target.value)} placeholder="Écrivez une note sur cet étudiant…" /><div className="drawer-note-footer"><span>Visible par l'équipe</span><Button icon="check" onClick={() => { if (note.trim()) { onAddNote(note); setNote(""); } }}>Ajouter la note</Button></div></div>{showRegistration && <NewRegistrationDrawer student={person} onClose={() => setShowRegistration(false)} onCreated={() => { setShowRegistration(false); setNotice(`Nouvelle inscription préparée pour ${person.name}.`); }} />}</Drawer>;
}

function NewRegistrationDrawer({ student, onClose, onCreated }: { student: Person; onClose: () => void; onCreated: () => void }) {
  const { courses, classes } = useData();
  const [formation, setFormation] = useState(courses[0].name);
  const matchingGroups = classes.filter((item) => item.subject.toLowerCase().includes(formation.split(" ")[0].toLowerCase()) || item.name.toLowerCase().includes(formation.split(" ")[0].toLowerCase()));
  const [group, setGroup] = useState(matchingGroups[0]?.name || classes[0].name);
  const [payment, setPayment] = useState("0");
  const course = courses.find((item) => item.name === formation) || courses[0];
  return <Drawer title="Nouvelle inscription" onClose={onClose}><div className="drawer-intro"><span className="activity-icon" style={{ background: COLORS.coralLight, color: COLORS.coral }}><Icon name="file" size={16} /></span><div><strong>{student.name}</strong><span>Créer une inscription en trois étapes simples.</span></div></div><div className="stepper"><span className="active">1. Formation</span><span>2. Groupe</span><span>3. Confirmation</span></div><div className="drawer-section"><h4>1. Choisir la formation</h4><label>Formation<select className="field" value={formation} onChange={(event) => setFormation(event.target.value)}>{courses.map((item) => <option key={item.id}>{item.name}</option>)}</select></label><label>Groupe<select className="field" value={group} onChange={(event) => setGroup(event.target.value)}>{(matchingGroups.length ? matchingGroups : classes).map((item) => <option key={item.id}>{item.name}</option>)}</select></label></div><div className="drawer-section"><h4>3. Confirmer les informations</h4><div className="info-grid"><div><span>Formation</span><strong>{course.name}</strong></div><div><span>Groupe</span><strong>{group}</strong></div><div><span>Date d'inscription</span><strong>15/09/2026</strong></div><div><span>Prix</span><strong>{formatMoney(course.price)}</strong></div></div><label>Paiement initial<input className="field" type="number" value={payment} onChange={(event) => setPayment(event.target.value)} /></label><label>Notes<textarea className="field textarea" placeholder="Informations utiles pour l'équipe…" /></label></div><div className="drawer-footer-actions"><Button variant="outline" onClick={onClose}>Annuler</Button><Button onClick={onCreated}>Confirmer l'inscription</Button></div></Drawer>;
}
