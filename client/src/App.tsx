...
function Pipeline({ pipeline, setPipeline, setSelected, setNotice }: { pipeline: Record<Stage, PipelineCard[]>; setPipeline: (next: Record<Stage, PipelineCard[]>) => void; setSelected: (card: PipelineCard) => void; setNotice: (message: string) => void }) {
  ...
  return <>
    <PageHeader eyebrow="Acquisition & conversion" title="Pipeline CRM" description="Visualisez chaque opportunité et faites avancer vos futurs étudiants." action actionLabel="Ajouter un prospect" onAction={() => setNotice("Nouveau prospect : formulaire prêt à être connecté à Supabase.")} />
    <div className="pipeline-summary"><div><span>Valeur du pipeline</span><strong>{formatMoney(Object.values(pipeline).flat().filter((card) => card.stage !== "abandonne").reduce((sum, card) => sum + card.value, 0))}</strong></div><div><span>Opportunités actives</span><strong>{Object.values(pipeline).flat().filter((card) => card.stage !== "abandonne" && card.stage !== "diplome").length}</strong></div><div><span>Temps moyen de conversion</span><strong>21 jours</strong></div><div className="pipeline-tip"><Icon name="spark" size={18} /><span>Glissez-déposez les cartes pour mettre à jour le statut.</span></div></div>
    <div className="pipeline-board">{stages.map((stage) => { const cards = pipeline[stage]; const total = cards.reduce((sum, card) => sum + card.value, 0); return <div className="pipeline-column" key={stage} onDragOver={(event) => event.preventDefault()} onDrop={() => moveCard(stage)}><div className="pipeline-column-head"><div><span className="stage-dot" style={{ background: stageColors[stage] }} /><strong>{stageLabels[stage]}</strong><span className="count-chip">{cards.length}</span></div><span className="column-total">{formatMoney(total).replace(" DA", "")}</span></div><div className="pipeline-cards">{cards.map((card) => <div key={card.id} draggable onDragStart={() => setDragged({ id: card.id, stage })} onClick={() => setSelected(card)} className="pipeline-card"><div className="pipeline-card-top">{initialsBadge(card.name)}<button className="icon-button" onClick={(event) => { event.stopPropagation(); setNotice(`Options de ${card.name}`); }}><Icon name="dots" size={16} /></button></div><strong>{card.name}</strong><span className="pipeline-course">{card.course}</span><div className="pipeline-card-foot"><span><Icon name="clock" size={12} /> {card.days === 0 ? "Nouveau" : `${card.days} j`}</span><span>{initialsBadge(card.staff)}</span></div></div>)}</div><button className="column-add" onClick={() => setNotice(`Ajouter une opportunité dans « ${stageLabels[stage]} »`)}><Icon name="plus" size={14} /> Ajouter</button></div> })}</div>
  </>;
}
